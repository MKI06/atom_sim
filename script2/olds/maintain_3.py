...
# Define a threshold area to filter out small contours (you may need to adjust this value)
threshold_area = 5000

# Filter out small contours (quarter circle)
filtered_contours = [cnt for cnt in contours if cv2.contourArea(cnt) > threshold_area]

# Find the minimum bounding rectangle that encloses all the remaining contours
rect = cv2.minAreaRect(np.concatenate(filtered_contours))
